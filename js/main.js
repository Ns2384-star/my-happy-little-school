...
/* ===== Balloons Animation ===== */
function initBalloonsAnimation() {
    // Configuration
    const config = {
        numberOfBalloons: 25,
        colors: [
            ...
            '#FFE066', // Sunshine
            '#F7CAC9', // Rose quartz
            '#6B5B95', // Purple primary
        ],
        minSize: 40,
        maxSize: 80,
        animationDuration: { min: 4000, max: 7000 },
        delayBetweenBalloons: 100,
        totalDuration: 8000 // Temps total avant que tout disparaisse
    };
    ...
    for (let i = 0; i < config.numberOfBalloons; i++) {
        setTimeout(() => {
            createBalloon(container, config, i);
        }, i * config.delayBetweenBalloons);
    }
...
function createBalloon(container, config, index) {
    ...
    const wobbleAmount = 20 + Math.random() * 40; // Amplitude du mouvement latéral
    const wobbleSpeed = 1 + Math.random() * 2; // Vitesse du wobble

    // Create balloon body
    const balloonBody = document.createElement('div');
    balloonBody.className = 'balloon__body';
    balloonBody.style.backgroundColor = color;
    balloonBody.style.width = `${size}px`;
    balloonBody.style.height = `${size * 1.2}px`;

    // Add shine effect
    const shine = document.createElement('div');
    shine.className = 'balloon__shine';
    balloonBody.appendChild(shine);

    // Create knot
    const knot = document.createElement('div');
    knot.className = 'balloon__knot';
    knot.style.borderTopColor = color;
    ...
    balloon.style.setProperty('--wobble-amount', `${wobbleAmount}px`);
    balloon.style.setProperty('--wobble-speed', `${wobbleSpeed}s`);
    balloon.style.setProperty('--float-duration', `${duration}ms`);
